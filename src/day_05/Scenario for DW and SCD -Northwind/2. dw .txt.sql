-- 2. creates the structure of the database NorthWindDw
...
(
	...
	RowEndDate DATE DEFAULT '9999-12-31' NOT NULL,
	RowChangeReason varchar(200) NULL,
	CONSTRAINT PK_ProductKey PRIMARY KEY CLUSTERED (ProductKey)
);
...
create table FactSales(
	...
	DiscountAmount FLOAT DEFAULT 0 NOT NULL,
...
DECLARE @StartDate DATETIME = '1996-01-01' --Starting value of Date Range
DECLARE @EndDate DATETIME = '1998-12-31' --End Value of Date Range
...
alter table [NorthwindDW].[dbo].factsales
	add constraint factsales_dimcustomer_customerkey_fk
		foreign key (customerkey) references [NorthwindDW].[dbo].dimcustomer (customerkey);

alter table [NorthwindDW].[dbo].factsales
	add constraint factsales_dimemployee_employeekey_fk
		foreign key (employeekey) references [NorthwindDW].[dbo].dimemployee (employeekey);

alter table [NorthwindDW].[dbo].factsales
	add constraint factsales_dimproduct_productkey_fk
		foreign key (productkey) references [NorthwindDW].[dbo].dimproduct (productkey);
...
alter table [NorthwindDW].[dbo].factsales
	add constraint factsales_dimdate_date_dim_id_fk
		foreign key (orderdatekey) references [NorthwindDW].[dbo].[DimDate] (DateKey);

alter table [NorthwindDW].[dbo].factsales
	add constraint factsales_dimdate_date_dim_id_fk_2
		foreign key (shippeddatekey) references [NorthwindDW].[dbo].[DimDate] (DateKey);
